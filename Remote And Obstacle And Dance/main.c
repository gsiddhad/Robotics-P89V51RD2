...
void turnleft(void) {			//Step sequence for turning Letf
	Servo_Control(R_M,RBAK);
...
void obstacle(void) {
	...
	while(esc==0) {
		...
		if(button == MD) {
			esc=1;
			break;
		} else if(Lobs==1 && Robs==1)
			goforward();
		else if(Lobs==0 && Robs==1)
			turnright();
		else if(Lobs==1 && Robs==0)
			turnleft();
		else if(Lobs==0 && Robs==0) {
			gobackward();
			gobackward();
			gobackward();
			gobackward();
...
//printf("\rDance A");
	...
	Servo_Control(R_M,RFWD);  Servo_Control(C_M,CNEU);   Servo_Control(L_M,LBAK);
   	delay1();
	Servo_Control(R_M,RNEU);  Servo_Control(C_M,CNEU);   Servo_Control(L_M,LNEU);
   	delay();
	Servo_Control(R_M,RBAK);  Servo_Control(C_M,CNEU);   Servo_Control(L_M,LFWD);
   	delay1();
	Servo_Control(R_M,RNEU);  Servo_Control(C_M,CNEU);   Servo_Control(L_M,LNEU);
   	delay();

    for (j=0;j<3;j++) {
		Servo_Control(R_M,RNEU);  Servo_Control(C_M,CRIT);   Servo_Control(L_M,LBAK);
   		delay1();
        Servo_Control(R_M,RNEU);  Servo_Control(C_M,CRIT);   Servo_Control(L_M,LFWD);
   		delay();
    }

    for (k=0;k<3;k++) {
        Servo_Control(R_M,RFWD);  Servo_Control(C_M,CLFT);   Servo_Control(L_M,LNEU);
   		delay1();
        Servo_Control(R_M,RBAK);  Servo_Control(C_M,CLFT);   Servo_Control(L_M,LNEU);
   		delay();
   	}

   	for (l=0;l<6;l++) {
        Servo_Control(R_M,RNEU);  Servo_Control(C_M,CLFT);   Servo_Control(L_M,LNEU);
   		delay2();
        Servo_Control(R_M,RNEU);  Servo_Control(C_M,CRIT);   Servo_Control(L_M,LNEU);
   		delay2();
   	}
...
//printf("\rDance B");
   	for(i=0;i<2;i++) {
        Servo_Control(R_M,RFWD);  Servo_Control(C_M,CNEU);   Servo_Control(L_M,LBAK);
   		delay1();
        Servo_Control(R_M,RBAK);  Servo_Control(C_M,CNEU);   Servo_Control(L_M,LBAK);
   		delay1();
   	}
   	...
   	for (k=0;k<4;k++) {
        Servo_Control(R_M,RFWD);  Servo_Control(C_M,CLFT);   Servo_Control(L_M,LFWD);
   		delay1();
      	Servo_Control(R_M,RFWD);  Servo_Control(C_M,CRIT);   Servo_Control(L_M,LFWD);
   		delay1();
   	}

   	for (l=0;l<4;l++) {
        Servo_Control(R_M,RBAK);  Servo_Control(C_M,CLFT);   Servo_Control(L_M,LBAK);
   		delay1();
      	Servo_Control(R_M,RBAK);  Servo_Control(C_M,CRIT);   Servo_Control(L_M,LBAK);
   		delay1();
   	}
