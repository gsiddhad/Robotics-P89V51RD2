...
void main(void) {
    ...
    while(1) {  //while 1 to repeat code again and again :-)
        button = IR_REMOTE();				// scan and check for any remote key is pressed
        if(button == FWD) goforward();		//if FWD key is preesed go forward
        if(button == BWD) gobackward();	//if BWD key is preesed go backward
        if(button == RIGHT) turnright(); 	//if RIGHT key is preesed turn right
        if(button == LEFT) turnleft();		//if LEFT key is preesed turn left
    }
}
